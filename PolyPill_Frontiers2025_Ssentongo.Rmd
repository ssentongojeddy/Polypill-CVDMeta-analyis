...
```
### Please credit the original author, Paddy Ssentongo, if you use this code in your work.
...
```{r,warning=FALSE,echo=FALSE, message=FALSE}
rm(list=ls())
dat=read.csv("data/Frontiers_2025_Polypill_Spreadsheet_2024.csv")
```
...
summary(dat$Adherence_Last_follow_up_usual_care_.,na.rm  =T)#adherence Usual Care
summary(dat$Adherence_Last_follow_up_Polypill_.,na.rm  =T)#adherence POLYPILL
...
summary(dat$Decrease_Blood_pressure_SBP_Polypill,na.rm  =T)#SBP_Poly
summary(dat$Decrease_Blood_pressure_SBP_SOC,na.rm  =T)#SBP_SOC
summary(dat$Decrease_Blood_pressure_SBP_delta,na.rm  =T)#SBP_delta
...
# DBP
```{r}
summary(dat$Decrease_Blood_pressure_DBP_Polypill,na.rm  =T)#DBP_Poly
summary(dat$Decrease_Blood_pressure_DBP_SOC,na.rm  =T)#DBP_SOC
summary(dat$Decrease_Blood_pressure_DBP_delta,na.rm  =T)#DBP_DELTA

 #Min. 1st Qu.  Median    Mean 3rd Qu.    Max.    NA's 
  #0.000   1.325   1.900   3.423   4.800  10.500       9 
```
...
summary(dat$Decrease_LDL_polypill,na.rm  =T)#LDL_Poly
summary(dat$Decrease_LDL_polypill_SOC,na.rm  =T)#LDL_SOC
summary(dat$Decrease_LDL_delta,na.rm  =T)#LDL_Delta
...
l1=is.na(l$Decrease_Blood_pressure_SBP_Polypill)
dat=l[!l1,]
...
h <- dat%>%
  ggplot(aes(x = Outcome, y = Decrease_Blood_pressure_SBP_Polypill)) +
...
group_by(dat, dat$Outcome) %>%
  summarise(
    count = n(),
    median = median (Decrease_Blood_pressure_SBP_Polypill, na.rm = TRUE),
    mean = mean(Decrease_Blood_pressure_SBP_Polypill, na.rm = TRUE),
    sd = sd(Decrease_Blood_pressure_SBP_Polypill, na.rm = TRUE),
    IQR = IQR(Decrease_Blood_pressure_SBP_Polypill, na.rm = TRUE)
  )
...
group_by(dat, dat$Outcome) %>%
  summarise(
    count = n(),
    median = median (Decrease_Blood_pressure_SBP_delta, na.rm = TRUE),
    mean = mean(Decrease_Blood_pressure_SBP_delta, na.rm = TRUE),
    sd = sd(Decrease_Blood_pressure_SBP_delta, na.rm = TRUE),
    IQR = IQR(Decrease_Blood_pressure_SBP_delta, na.rm = TRUE)
  )
...
l1=is.na(l$Decrease_Blood_pressure_DBP_Polypill)
dat=l[!l1,]
...
hh <- dat%>%
  ggplot(aes(x = Outcome, y = Decrease_Blood_pressure_DBP_Polypill)) +
  #ggplot(aes(x = reorder(times, Rates2, FUN = median, .desc =TRUE, na.rm=T), y = Rates2))+ 
  #geom_smooth(se = F, method = "lm", size = 1.5) +
  geom_boxplot(outlier.shape = NA,outlier.size =0)+
  geom_jitter(color="black", size=0.9, alpha=0.4, shape=5) +
...
group_by(dat, dat$Outcome) %>%
  summarise(
    count = n(),
    median = median (Decrease_Blood_pressure_DBP_Polypill, na.rm = TRUE),
    mean = mean(Decrease_Blood_pressure_DBP_Polypill, na.rm = TRUE),
    sd = sd(Decrease_Blood_pressure_DBP_Polypill, na.rm = TRUE),
    IQR = IQR(Decrease_Blood_pressure_DBP_Polypill, na.rm = TRUE)
  )
...
group_by(dat, dat$Outcome) %>%
  summarise(
    count = n(),
    median = median (Decrease_Blood_pressure_DBP_delta, na.rm = TRUE),
    mean = mean(Decrease_Blood_pressure_DBP_delta, na.rm = TRUE),
    sd = sd(Decrease_Blood_pressure_DBP_delta, na.rm = TRUE),
    IQR = IQR(Decrease_Blood_pressure_DBP_delta, na.rm = TRUE)
  )
```
...
l1=is.na(l$Decrease_LDL_polypill)
dat=l[!l1,]
...
hhh <- dat%>%
  ggplot(aes(x = Outcome, y = Decrease_LDL_polypill)) +
  #ggplot(aes(x = reorder(times, Rates2, FUN = median, .desc =TRUE, na.rm=T), y = Rates2))+ 
  #geom_smooth(se = F, method = "lm", size = 1.5) +
  geom_boxplot(outlier.shape = NA,outlier.size =0)+
  geom_jitter(color="black", size=0.9, alpha=0.4, shape=5) +
...
group_by(dat, dat$Outcome) %>%
  summarise(
    count = n(),
    median = median (Decrease_LDL_polypill, na.rm = TRUE),
    mean = mean(Decrease_LDL_polypill, na.rm = TRUE),
    sd = sd(Decrease_LDL_polypill, na.rm = TRUE),
    IQR = IQR(Decrease_LDL_polypill, na.rm = TRUE)
  )
```
...
tiff("figs/Fig6.tiff", width=15000, height=6000, res=600, compression = "lzw")
